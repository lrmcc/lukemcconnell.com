...
function initGrid() {
    grid.xAxis = innerWidth * 0.21;
    grid.yAxis = innerHeight * 0.35;
}
...
function mobileInitSizing() {
    ...
    page.yPos = [grid.yAxis * 0.4, grid.yAxis * 1.1, grid.yAxis * 1.8, grid.yAxis * 2.7];
    if (innerWidth < 420) {
        ...
        page.yPos = [grid.yAxis * 0.4, grid.yAxis * 1.14, grid.yAxis * 1.88, grid.yAxis * 2.66];
    }
}
...
function drawDevIcon() {
    ...
    page.objects.push(new Line(page.xPos[0] + (page.iconWidth * 0.35), page.yPos[0] + page.iconHeight[0], page.xPos[0] + (page.iconWidth * 0.25), page.yPos[0] + (page.iconHeight[0] * 1.15), "round", page.lineWidth, page.strokeStyle));
    page.objects.push(new Line(page.xPos[0] + (page.iconWidth * 0.65), page.yPos[0] + page.iconHeight[0], page.xPos[0] + (page.iconWidth * 0.75), page.yPos[0] + (page.iconHeight[0] * 1.15), "round", page.lineWidth, page.strokeStyle));
    ...
    let starXFactor = [0.1, 0.3333, 0.5666, 0.8, 0.1, 0.3333, 0.5666, 0.8, 0.1, 0.1, 0.8, 0.8];
    let starYFactor = [0.12, 0.12, 0.12, 0.12, 0.85, 0.85, 0.85, 0.85, 0.32, 0.64, 0.32, 0.64];
...
function checkOverIcon() {
    ...
    for (let i = 0; i < 4; i ++) {
        ...
        if (mouse.x > iconXStart && mouse.x < iconXEnd && mouse.y > iconYStart && mouse.y < iconYEnd) {
            mouse.isOverIcon = true;
            mouse.icon = text.toLowerCase();
            ctx.beginPath();
...
function SpeechBubble(x, y, w, h) {
    ...
    this.draw = function() {
        ...
        ctx.bezierCurveTo(this.x, this.y - (0.1 * this.w),          // top left corner
            this.x + (0.1 * this.w), 
            this.y - (0.1 * this.w), 
            this.x + (0.1 * this.w), 
            this.y - (0.1 * this.w));                              
...
function Fuji(moveToX, moveToY, lineToX, lineToY) {
    ...
    this.draw = function() {
        ...
        ctx.moveTo(this.moveToX + (page.iconWidth * 0.01), this.moveToY);
        ctx.quadraticCurveTo(this.moveToX + (page.iconWidth * 0.25), this.moveToY - (page.iconHeight[2] * 0.15), this.lineToX - (page.iconWidth * 0.01), this.lineToY);
...
function Star(x, y, size) {
    ...
    this.fillStyle = starColors[Math.floor(Math.random() * starColors.length)];
    this.timer = Date.now();
...
function BlogLines() {
    ...
    let lineXFactor = [0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.5];
    ...
    this.update = function() {
        this.draw();
        if (Date.now() - this.timer > this.timeLimit) {
            if (numberLines < moveXFactor.length + 1) {
                numberLines++;
                this.timeLimit = (Math.random() * 1000) + 200;
                this.timer = Date.now();
            }
            if (numberLines == moveXFactor.length + 1) {
                if (Date.now() - this.timer > 2400) {
                    numberLines = 0;
                    this.timer = Date.now();
                }
            }
...
function Pencil() {
    ...
    let moveYFactor = [0.12, 0.13, 0.69, 0.7, 0.12, 0.18];
    let lineXFactor = [0.74, 0.82, 0.775, 0.775, 0.87, 0.86];
    let lineYFactor = [0.69, 0.7, 0.72, 0.72, 0.128, 0.188];
    ...
    this.draw = function() {
        ...
        ctx.beginPath();
        ctx.lineCap = "butt";
        ctx.lineWidth = page.lineWidth * 1.5;
        ctx.strokeStyle = "#bf647f";
        ctx.moveTo(page.xPos[1] + (page.iconWidth * 0.834), page.yPos[1] + (page.iconHeight[1] * 0.11));
        ctx.lineTo(page.xPos[1] + (page.iconWidth * 0.825), page.yPos[1] + (page.iconHeight[1] * 0.18)); // end here
        ctx.stroke();
        ...
        ctx.lineCap = "butt";
        ctx.lineWidth = page.lineWidth;
        ctx.strokeStyle = "#ad7e50";
        ctx.moveTo(page.xPos[1] + (page.iconWidth * 0.775), page.yPos[1] + (page.iconHeight[1] * 0.66)); //start here
        ctx.lineTo(page.xPos[1] + (page.iconWidth * 0.77), page.yPos[1] + (page.iconHeight[1] * 0.7));
        ctx.stroke();

        // Inner Yellow Line
        ctx.beginPath();
        ctx.strokeStyle = '#fbbf19';
        ctx.lineCap = "butt";
        ctx.lineWidth = page.lineWidth;
        ctx.moveTo(page.xPos[1] + (page.iconWidth * 0.825), page.yPos[1] + (page.iconHeight[1] * 0.18));
        ctx.lineTo(page.xPos[1] + (page.iconWidth * 0.78), page.yPos[1] + (page.iconHeight[1] * 0.665));
        ctx.stroke();
